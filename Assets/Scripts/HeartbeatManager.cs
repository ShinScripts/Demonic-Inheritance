...
public void UpdateEnemyDistance() {
    ...
    private void UpdateHeartbeatDelay() {
        float distanceFactor = Mathf.Lerp(minHeartbeatDelay, maxHeartbeatDelay, normDistanceEnemy);

        heartbeatDelay = distanceFactor / (isPlayerSpotted ? 1.65f : 1);
        if(heartbeatDelay < 0.45f) {
            heartbeatDelay = 0.45f;
        }
    }
